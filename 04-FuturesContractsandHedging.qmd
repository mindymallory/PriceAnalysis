...
The farmer will sell corn in the cash market in November, so to hedge she should sell (the December) futures to hedge. This concept is called being natually long. Since the farmer is essentially long the unharvested grain in the field (after all, 'long' is a financial position that gains value when price goes up and loses value when price goes down). Therefore, to hedge, they need to take a position with the opposite profile, i.e., they should take a short position to hedge their naturally long position.

Consider two cases, one where the Dec futures price in November turned out to be \$4.00, and one where the Dec futures price in November turned out to be \$3.60.
