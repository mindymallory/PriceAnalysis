...
![Figure 1: Regressing Hours Doing Homework on Hours Exercising](MT_Cars_Regression_files/image005.png)
...
![Figure 2: Linear Regression Minimizes the Sum of the Squared Residuals, $\epsilon_i$](MT_Cars_Regression_files/image026.png)

The predicted values of $Y$ are denoted by $\hat{Y}$, so stating the linear regression problem formally, 

$$\underset{\beta_0, \beta_1}{Min} \sum_{i=1}^{T} (Y_i - \hat{Y})^2$$ 

or equivalently, 

$$\underset{\beta_0, \beta_1}{Min} \sum_{i=1}^{T} [Y_i - (\beta_0 + \beta_1X_i)]^2$$ 
...
![Figure 4: Illustrateing the *t*-test](MT_Cars_Regression_files/image027.png)
...
- This phenomenon is called a **Spurious Regression**. 
- This means that if you unknowingly regress a non-stationary variable on another non-stationary variable and perform a *t*-test to see if they are related, you will almost always conclude that they are because the *t*-statistic will be very large compared to the *t* distribution. 
...
![Figure 5: Simulated Uncorrelated Log-normal Prices](MT_Cars_Regression_files/image017.png)
...
- In general, we will not know for sure if our price series is stationary or not. 
- However, in most cases using the log-difference of price instead of price levels will transform the non-stationary series into a stationary one, so regression-based forecasting methods can safely be done by regressing log-differences of price in various forms. 
...
- There are statistical tests to detect if a series is stationary or not, and a more advanced analysis would test for stationarity as an initial exploration to help determine what kind of forecasting model to specify. 
- Some examples of stationarity tests are the Augmented Dickey-Fuller test [@dickey1979distribution], the Phillips-Perron test [@phillips1988testing], and the Kwiatkowski–Phillips–Schmidt–Shin (KPSS) test [@kwiatkowski1992testing]. 
- These are not convenient to implement in Excel, so we will just use a standard practice of using log-differences of the price series we try to forecast. 
