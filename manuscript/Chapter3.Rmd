...
Conducing fundamental analyses involves taking into account all the factors that determine supply, demand, and ultimately, prices. Recall the supply and demand figures from chapter 1. They are the basic theoretical framework that will guild all of our modeling decisions. For grain markets we had two models: preplanting and post planting. 
...
In the next few tables below, we outline the typical format for balance sheet analysis. 
...
: Table 4. Balance Sheet for Corn

The balance sheet for corn follows the same generic patter, but we can be a bit more specific with the use categories since we know what the major use catagories are for any given commodity. The use components are as follows: 
<!-- Put percentages here? --> 

- Feed and Residual: A large portion of the corn crop is used as feed for livestock (cattle, pigs, poultry).
- Food, Seed, and Industrial: Corn is used to make tortilla chips, high fructose corn syrup, edible oil and other food items. A portion of the crop is grown specifically as seed for the next years crop. There are some industural uses for components of processed corn. Those are grouped in this category as well. 
- Ethanol production also demands a significant amount of corn, so much that it gets its own category in the balance sheet. 
...
: Table 5. Balance Sheet for Soybeans

Describe soybeans here. 
