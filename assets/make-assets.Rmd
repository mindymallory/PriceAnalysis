...
library(httr)
library(tidyr)
tmp = 'C:/Users/mindy/Desktop/Documents/PriceAnalysis/PriceAnalysis/assets/OilCropsAllTables.xlsx'

data_xl <- read_excel(tmp, sheet = "tab3", 
                      skip = 7, 
                      col_names = c("MktYear", "BeginningStocks", "Production", "Imports", 
                                    "TotalSupply", "Crush", "Exports", "Seed", "TotalUse", "EndingStocks", "SeasonAvePrice")) %>%
  mutate(SeasonAvePrice = as.numeric(SeasonAvePrice)) 
  #fill(MktYear) %>% 
  # # Make columns for more standard date handling
  # mutate(month = ifelse(Quarter =="Q1 September-November", "09-01", 
  #                       ifelse(Quarter == "Q2 December-February", "12-01", 
...
Now let's plot the soy dissapearance

```{r, fig.height= 5}
# Plot the Marketing Year Data
data_tidy %>% 
  #filter(Quarter != "MY September-August") %>%
  mutate(MktYear = as.integer(str_sub(MktYear, 1, 4))) %>%  # "1980/81" -> 1980
  filter(UseCategory %in% c("Crush", "Exports")) %>%
  ggplot(aes(x = MktYear, y = Value, color = UseCategory)) + 
  geom_line() + 
  theme_bw() +
  labs(x = "", y = "Millions of bushels", title = "Soybean Quarterly Use Categories Since 1975")
ggsave(filename = "ForecastingUseof-SoyUseCategories.png")
...
data_tidy %>% 
  ...
  mutate(MktYear = as.integer(str_sub(MktYear, 1, 4))) %>%  # "1980/81" -> 1980
  filter(UseCategory %in% c("Exports", "TotalUse")) %>%
  filter(!is.na(MktYear)) %>%
  pivot_wider(names_from = UseCategory, values_from = Value) %>%
  mutate(ProportionofUse = Exports/TotalUse) %>%
  ggplot(aes(x = MktYear, y = ProportionofUse)) + 
  geom_line(color = purduegold, size = .75) + 
  theme_bw() +
  labs(x = "", y = "Millions of bushels", title = "Soybean Marketing Year Exports as Proportion of Total Use")
ggsave(filename = "ForecastingUseof-SoyUseCategoriesExportsPropofUse.png")

data_tidy %>% 
  #filter(Quarter == "MY September-August") %>%
  mutate(MktYear = as.integer(str_sub(MktYear, 1, 4))) %>%  # "1980/81" -> 1980
  filter(UseCategory %in% c("Crush", "TotalUse")) %>%
  filter(!is.na(MktYear)) %>%
  pivot_wider(names_from = UseCategory, values_from = Value) %>%
  mutate(ProportionofUse = Crush/TotalUse) %>%
  ggplot(aes(x = MktYear, y = ProportionofUse)) + 
  geom_line(color = purduegold, size = .75) + 
  theme_bw() +
  labs(x = "", y = "Millions of bushels", title = "Soybean Marketing Year Crush as Proportion of Total Use")
ggsave(filename = "ForecastingUseof-SoyUseCategoriesCrushPropofUse.png")
...
prop_long <- data_tidy %>%
  mutate(MktYear = as.integer(str_sub(MktYear, 1, 4))) %>%
  filter(!is.na(MktYear)) %>%
  filter(UseCategory %in% c("Exports", "Crush", "TotalUse")) %>%
  group_by(MktYear, UseCategory) %>%
  summarise(Value = sum(Value, na.rm = TRUE), .groups = "drop") %>%  # avoids list-cols if duplicates exist
  pivot_wider(names_from = UseCategory, values_from = Value) %>%
  transmute(
    MktYear,
    Exports = Exports / TotalUse,
    Crush   = Crush   / TotalUse
  ) %>%
  pivot_longer(cols = c(Exports, Crush),
               names_to = "UseCategory",
               values_to = "ProportionofUse")

ggplot(prop_long, aes(x = MktYear, y = ProportionofUse, color = UseCategory)) +
  geom_line(size = 0.75) +
  theme_bw() +
  labs(x = "", y = "Proportion of Total Use",
       title = "Soybean Marketing Year Use as Proportion of Total Use") +
  scale_color_manual(values = c(Exports = purduegold, Crush = "black"))

ggsave("ForecastingUseof-SoyUseCategories_Exports_Crush_PropofUse.png",
